...
float HistogramBuilderTwo::WrapCheck(float phi1, float phi2){
  ...
  // Use the float modulus operator, Warning takes the same sign 
  //Change to 0 to 2Pi range.

  float pos_angle = fabs(delta_phi);
  float delta_mod = fmod(pos_angle, 2*PI) - PI;
  ...
  float wrap_dif = delta_mod - delta_case;
  float absWrap_dif = abs(wrap_dif);
  if( absWrap_dif > 0.0001){
    std:: cout << "Not equal to 0" << std::endl;
    std::cout << delta_mod - delta_case << std::endl;
    std::cout << delta_phi << std::endl;
    std::cout << pos_angle << std::endl;
    std::cout << delta_mod << std::endl;
    std::cout << delta_case << std::endl;
